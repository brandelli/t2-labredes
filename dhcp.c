...
// inicialização das variaveis utilizadas na aplicação---------------------------------------------
unsigned char buff1[BUFFSIZE]; // buffer de recepcao
const int time_lease = 86400; // 1 dia
const __u32 subnet_mask = 0x00FFFFFF; //255.255.255.0
char interface_name[] = "wlp2s0";
char current_ip = 100;
...
//estrutura para guardas as informações do bootstrap protocol
struct boothdr
{
   char msg_type;
   char hdr_type;
...
void dhcp_boot_reply_action(char reply_type)
{
   ...
   client_ip = client_ip | (current_ip << 24);
   ...
   booth->options[option_index++] = reply_type & 0xFF;
...
int main(int argc,char *argv[])
{
   ...
   strncpy(ifr.ifr_name, interface_name, (int) strlen(interface_name));
   ...
   strncpy(if_idx.ifr_name, interface_name, (int) strlen(interface_name));
   if (ioctl(sockd, SIOCGIFINDEX, &if_idx) < 0)
       perror("SIOCGIFINDEX");

   memset(&if_mac, 0, sizeof(struct ifreq));
   strncpy(if_mac.ifr_name, interface_name, (int) strlen(interface_name));
   if (ioctl(sockd, SIOCGIFHWADDR, &if_mac) < 0)
       perror("SIOCGIFHWADDR");

   memset(&if_ip, 0, sizeof(struct ifreq));
   strncpy(if_ip.ifr_name,  interface_name, (int) strlen(interface_name));
      ...
      switch (dhcp_status)
      {
         case DHCP_DISCOVER:
            while(dhcp_status == DHCP_DISCOVER){
               printf("esperando DHCP_DISCOVER\n");
               recv(sockd,(char *) &buff1, sizeof(buff1), 0x0);
               //verificação de pacote udp, com porta origem 68 e porta destino 67 
               if(iph->protocol == 17 && udph->source == htons(68) && udph->dest == htons(67)){
                  option_code = booth->options[0] & 0xFF;
                  option_type = booth->options[2] & 0xFF;
                  if(option_code == 53 && option_type == 1){
                     printf("DHCP_DISCOVER\n");
                     dhcp_boot_request_action(option_code, option_type);
